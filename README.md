...
- 多人音频拆分：历史 Session、说话人校对、自动去静音、可裁剪波形音轨、原片段恢复，以及全局/逐段音轨显示开关。
...
长会话每位对话人每页显示 30 段，可搜索整个 Session 的台词、按对话人筛选；筛选和翻页不丢失校对草稿。“保存全部修改”会原子保存所有页的修改，有任意无效片段则全部不写入。导出前必须先保存，刷新或关闭有未保存修改的页面时会提醒。预计人数会传给主要的 CAM++ 聚类；被重叠语音排除的整句也会保留为待复核片段（新分析生效），不会自动用于克隆。

新拆分会自动清理片段首尾静音和超过 0.5 秒的句内空白，保留短停顿与字头、字尾余量；历史结果可点击“全 Session 一键去静音”。导出音色时也会再次清理空白，纯静音不会进入克隆素材，时长限制按清理后的实际音频计算。清理使用本地能量检测，不额外加载模型或占用 GPU；它不是降噪或人声分离，底噪较大的录音仍需试听校对。

每段下方的“片段音轨”显示真实波形，支持拖动两端或输入精确秒数、试听选区和“应用裁剪”。音频操作立即保存为新文件，原始录音及原片段始终保留，可用“恢复原片段”撤销裁剪。裁剪或恢复后，请核对台词、勾选确认并“保存修改”，再导出克隆音色，避免声音与参考文本不一致。波形按可见区域加载，切换 Session 不会混用音轨或裁剪选区。

音轨有两级显示开关：

- 顶部筛选栏的“隐藏全部音轨 / 显示全部音轨”统一设置所有片段，包括其他分页，并在本浏览器中记忆。使用全局开关会重置单段显示选择。
- 每个片段时间信息下方的“隐藏本段音轨 / 显示本段音轨”只控制当前片段；即使已全部隐藏，也能单独展开一段。单段选择在当前页面内按 Session 隔离保存，筛选、翻页或切换会话不会混用；刷新后恢复全局偏好。

隐藏会暂停对应音轨的试听，不影响未保存的台词、裁剪选区或后台处理。开关直接更新显示状态，不会重新渲染整个会话。
...
音色库保留参考录音与准确文字；生成新台词时提取并复用说话人向量和参考语音编码，不会为每句台词重新训练模型。当前 Qwen3-TTS Base 适配器已有内存中的 `voice_clone_prompt` 缓存，同模型连续生成可复用；重启工作器、切换模型或执行多人拆分会卸载模型并清空缓存。尚未实现跨重启的磁盘特征缓存，参考编码仍参与每次合成，所以参考录音并非越长越好。

Base 克隆不支持 `instruct` 情绪指令，界面上的情绪标注不保证在该模型中生效；它主要继承参考录音的风格，语速通过生成后的 FFmpeg 变速处理。VoiceDesign 的提示词设计与 Base 的克隆属于不同能力。模型能力见 [Qwen3-TTS 官方说明](https://github.com/QwenLM/Qwen3-TTS#released-models-description-and-download)。
